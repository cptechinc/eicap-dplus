...
trait DocumentManagementAR {
	 ...
	 * @return string
	 */
	public function filter_arpayment_conditions(DocumentQuery $documents_master, $invnbr, $checknbr = '') {
		...
		if ($checknbr) {
			...
			$documents_master->condition('tag_customer', "Document.$column_tag = ?", self::TAG_CUSTOMER);
			$documents_master->condition('reference1_customer', "Document.$column_reference1 = ?", $custID);
			$documents_master->combine(array('tag_customer', 'reference1_customer'), 'and', 'cond_customer');

			// Create Customer Checks Filter
			$documents_master->condition('tag_customer_check', "Document.$column_tag = ?", self::TAG_CUSTOMER);
			$documents_master->condition('reference1_customer_check', "Document.$column_reference1 = ?", $custID);
			$documents_master->condition('tag_checks', "Document.$column_tag = ?", self::TAG_AR_CHECKS);
			$documents_master->condition('reference2_checks', "Document.$column_reference2 = ?", $checknbr);
